...
async function CanvasHerokuCompatibilityTest() {
	const guild = client.guilds.cache.first();
	const member = guild.me;
	const testing_grounds = guild.channels.cache.get(constants.channels.qg.testing_ground_text);
	...
	await testing_grounds.send(`Welcome to the server, ${member}!`, attachment);
}
...
client.once('ready', async () => {
	...
	try {
		...
		uploadLocalEmojis();
		await CanvasHerokuCompatibilityTest();
	}
	catch (error) {
...
client.on('userUpdate', (oldUser, newUser) => {
	if (!app.isInitialized() || !app.member(newUser)) return;
	try {
...
client.on('guildMemberRemove', (this_member) => {
	if (!app.isInitialized() || this_member.guild.id !== constants.guild) return;
	try {
		const created_day = this_member.user.createdAt;
		const created_day_difference = functions.compareDate(created_day);
...
client.on('guildBanAdd', (this_guild, this_user) => {
	if (!app.isInitialized() || this_guild.id !== constants.guild) return;
	try {
		const created_day = this_user.createdAt;
		const created_day_difference = functions.compareDate(created_day);
...
client.on('guildBanRemove', (this_guild, this_user) => {
	if (!app.isInitialized() || this_guild.id !== constants.guild) return;
	try {
		const created_day = this_user.createdAt;
		const created_day_difference = functions.compareDate(created_day);
...
client.on('roleCreate', (this_role) => {
	if (!app.isInitialized() || this_role.guild.id !== constants.guild || functions.contains(this_role.name, ['Play', 'Text', 'Team'])) return;
	try {
		const embed = new Discord.MessageEmbed();
...
client.on('roleUpdate', (oldRole, newRole) => {
	if (!app.isInitialized() || newRole.guild.id !== constants.guild || functions.contains(newRole.name, ['Play', 'Text', 'Team'])) return;
	try {
		const embed = new Discord.MessageEmbed();
		embed.setAuthor('Quarantine Gaming: Role Submanager');
...
client.on('inviteCreate', (invite) => {
	if (!app.isInitialized() || invite.guild.id !== constants.guild) return;
	try {
...
client.on('voiceStateUpdate', (oldState, newState) => {
	if (!app.isInitialized() || newState.guild.id !== constants.guild) return;
	try {
...
client.on('messageReactionAdd', async (reaction, user) => {
	if (!app.isInitialized() || reaction.message.guild.id !== constants.guild) return;
	try {
...
client.on('emojiCreate', (emoji) => {
	if (!app.isInitialized() || emoji.guild.id !== constants.guild) return;
	try {
		const embed = new Discord.MessageEmbed();
...
client.on('emojiUpdate', (oldEmoji, newEmoji) => {
	if (!app.isInitialized() || newEmoji.guild.id !== constants.guild) return;
	try {
		const embed = new Discord.MessageEmbed();
		embed.setAuthor('Quarantine Gaming: Emoji Submanager');
