const { ProcessQueue, contains, sleep, constants } = require('../utils/Base.js');
	...
	async reload() {
		...
		for (const member of members.filter(this_member => !this_member.user.bot)) {
			...
			for (const game_activity of game_activities) {
				...
				// Game Role
				const game_role = this.client.guild.roles.cache.find(role => role.name == game_name) || await this.client.role_manager.create({ name: game_name, color: constants.colors.game_role });
				...
				const play_role = this.client.guild.roles.cache.find(role => role.name == 'Play ' + game_name) || await this.client.role_manager.create({ name: 'Play ' + game_name, color: constants.colors.play_role, position: streaming_role.position, hoist: true });
