	...
	constructor(client) {
		...
		client.on('inviteCreate', async (invite) => {
			...
			try {
				...
				if (invite.expiresAt) description.push(`**Expires:** ${formatted_date}`);
		...
		client.on('inviteDelete', async (invite) => {
			...
			try {
				...
				if (invite.maxUses !== 1 || Date.now() >= invite.expiresTimestamp) this.invites.delete(invite.code);
			} catch (error) {
		...
		client.on('guildMemberAdd', async (member) => {
			...
			try {
				...
				const formatted_date = [
					`\`${(new Date(created_day)).toString().split(' ').splice(0, 5).join(' ')}\``,
		...
		client.on('guildMemberUpdate', async (oldMember, newMember) => {
			...
			try {
				...
				let message = messages.find(this_message => {
					const member = client.member(this_message.embeds[0]?.fields[0]?.value);
					return member?.id === newMember.id && this_message.embeds[0]?.fields[3]?.value === 'Pending';
				});

				if (!newMember.user.bot) {
					await client.message_manager.sendToUser(newMember, [
				...
				const action_message = await message.edit({
					content:'',
					embed: embed.setFooter('Apply actions by clicking one of the buttons below.'),
					components: this.client.interaction_manager.components.get('member_screening').getComponents(),
				});
				await action_message.reply(`${this.client.cs.roles.everyone}, ${newMember} wants to join the server.`);
			} catch (error) {
		...
		client.on('guildMemberRemove', async (member) => {
			...
			try {
				...
				const formatted_date = [
					`\`${(new Date(joined_day)).toString().split(' ').splice(0, 5).join(' ')}\``,
	...
	async ScreenMember(member, invite, showWelcome = true) {
		try {
			...
			if (showWelcome) {
				await this.client.message_manager.sendToUser(member, {
					content: [
						...
						'Before you will be given full access to this server, you must first complete the server\'s screening process.',
					].join('\n'),
				});
			}
			...
			const embed = new MessageEmbed({
				...
				thumbnail: { url: member.user.displayAvatarURL() },
				fields: [
					{ name: `Profile (${member.user.username}):`, value: member },
					{ name: `Inviter Profile (${invite?.inviter?.username ?? 'N\\A'}):`, value:  invite ? invite.inviter : 'No information' },
					{ name: `Account Created (${compareDate(created_day).estimate} ago):`, value: new Date(created_day) },
			...
			const message = messages.find(this_message => {
				const this_member = this.client.member(this_message.embeds[0]?.fields[0]?.value);
				return this_member?.id === member.id
			...
			return message
				...
				: await this.client.message_manager.sendToChannel(constants.cs.channels.gateway, embed);
		} catch (error) {
