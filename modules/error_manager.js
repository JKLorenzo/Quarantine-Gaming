        ...
        try {
            ...
            if ((epm > 5 || details.error.code == '500') && !threshold_reached) {
                ...
                await message.sendToChannel(constants.channels.server.announcements, `**Discord Status Updates**\nDiscord is currently having some issues and may impact users on this server. Visit <https://discordstatus.com/> for more info.`).catch(async () => {
                    ...
                    await message.sendToChannel(constants.channels.server.announcements, embed);
                });

                // Notify staffs
                await message.sendToChannel(constants.channels.server.management, `I'm currently detecting issues with Discord; some functionalities are disabled. A bot restart is recommended once the issues are resolved.`).catch(async () => {
                    const embed = new MessageEmbed();
                    embed.setAuthor('Limited Functionality');
                    ...
                    embed.setColor('ffe300');
                    await message.sendToChannel(constants.channels.server.management, embed);
                });
